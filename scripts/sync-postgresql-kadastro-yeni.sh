...
# =============================================================================
# TKGM PostgreSQL Kadastro Senkronizasyonu
# Kaynak PostgreSQL (tk_parsel) -> Hedef PostgreSQL (kadastro_yeni)
# JOIN'li sorgu ile veri zenginleştirme ve format dönüşümü
# =============================================================================
...
load_config() {
    ...
    POSTGRES_TARGET_PORT="${POSTGRES_TARGET_PORT:-5432}"
    ...
    POSTGRES_TARGET_TABLE="kadastro_yeni"
...
SELECT
    ...
    CAST(p.sistemguncellemetarihi AS TIMESTAMP WITH TIME ZONE) AS "m_date",
    CAST(p.tapumahalleref AS BIGINT) AS "tapu_mahalle_ref",
    CAST(m.tapumahallead AS VARCHAR(255)) AS "tapu_mahalle_adi",
    CAST(m.ilceref AS INTEGER) AS "ilce_id",
    CAST(i.ad AS VARCHAR(255)) AS "ilce_adi",
    CAST(p.tapuzeminref AS BIGINT) AS "zemin_id",
    CAST(p.tapucinsid AS BIGINT) AS "tapu_cins_id",
    CAST(p.tapucinsaciklama AS VARCHAR(1000)) AS "tapu_cins_aciklama",
    CAST(p.tip AS INTEGER) AS "tip",
    CAST(p.kmdurum AS INTEGER) AS "km_durum",
    CAST(kmd.adi AS VARCHAR(255)) AS "km_durum_aciklamasi",
    CAST(p.hazineparseldurum AS INTEGER) AS "hazine_parsel_durum",
    CAST(hpd.adi AS VARCHAR(255)) AS "hazine_parsel_durum_aciklama",
    CAST(p.durum AS INTEGER) AS "durum",
    CAST(d.adi AS VARCHAR(255)) AS "durum_aciklama",
    CAST(p.fid AS BIGINT) AS "fid",
    CAST(p.sistemkayittarihi AS TIMESTAMP WITH TIME ZONE) AS "sistem_kayit_tarihi",
    CAST(p.onaydurum AS INTEGER) AS "onay_durum",
    CAST(od.adi AS VARCHAR(255)) AS "onay_durum_aciklama",
    CAST(COALESCE(p.adano::text, '') || '-' || COALESCE(p.parselno::text, '') || '-' ||
         COALESCE(i.ad, '') || '-' || COALESCE(m.tapumahallead, '') AS VARCHAR) AS "apim",
    p.geom AS "geometry"
...
sync_data() {
    ...
    # Kaynak kayıt sayısı
    local source_count
    source_count=$(PGPASSWORD="$POSTGRES_SOURCE_PASS" psql -h "$POSTGRES_SOURCE_HOST" \
        -p "$POSTGRES_SOURCE_PORT" -U "$POSTGRES_SOURCE_USER" -d "$POSTGRES_SOURCE_DB" \
        -t -c "SELECT COUNT(*) FROM tk_parsel WHERE durum <> '2';" 2>/dev/null | \
        grep -E '[0-9]+' | head -1 | tr -d ' ' | xargs)
    log "Source record count (tk_parsel WHERE durum <> '2'): ${source_count:-unknown}"
    ...
    local target_count
    target_count=$(get_record_count "$POSTGRES_TARGET_HOST" "$POSTGRES_TARGET_PORT" \
        "$POSTGRES_TARGET_USER" "$POSTGRES_TARGET_PASS" "$POSTGRES_TARGET_DB" "$POSTGRES_TARGET_TABLE")

    if [ -n "$target_count" ] && [ "$target_count" -gt 0 ]; then
        log_success "Data transfer completed. Source: ${source_count:-?} -> Target: $target_count records"
    else
...
create_spatial_index() {
    ...
    PGPASSWORD="$POSTGRES_TARGET_PASS" psql -h "$POSTGRES_TARGET_HOST" \
        -p "$POSTGRES_TARGET_PORT" -U "$POSTGRES_TARGET_USER" -d "$POSTGRES_TARGET_DB" \
        -c "CREATE INDEX IF NOT EXISTS \"${POSTGRES_TARGET_TABLE}_GEOMETRY_geom_idx\" ON ${POSTGRES_TARGET_TABLE} USING GIST (geometry);" \
        2>&1 | tee -a "$LOG_FILE"
...
main() {
    load_config
    log "=== TKGM Kadastro PostgreSQL Sync Started ==="
    log "Source: ${POSTGRES_SOURCE_HOST}:${POSTGRES_SOURCE_PORT}/${POSTGRES_SOURCE_DB} (tk_parsel)"
    log "Target: ${POSTGRES_TARGET_HOST}:${POSTGRES_TARGET_PORT}/${POSTGRES_TARGET_DB} (${POSTGRES_TARGET_TABLE})"
